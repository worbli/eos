...
try:
    ...
    transId=node.createInitializeAccount(currencyAccount, cluster.eosioAccount, buyRAM=200000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.eosioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.eosioAccount, buyRAM=200000, waitForTransBlock=True, exitOnError=True)
