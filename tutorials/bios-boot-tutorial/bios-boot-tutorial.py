...
def startWallet():
    ...
    sleep(.4)
    run(args.worbli + 'wallet create --to-console')

def importKeys():
    run(args.worbli + 'wallet import --private-key ' + args.private_key)
    keys = {}
    for a in accounts:
        key = a['pvt']
        if not key in keys:
            if len(keys) >= args.max_user_keys:
                break
            keys[key] = True
            run(args.worbli + 'wallet import --private-key ' + key)
...
def createSystemAccounts():
    for a in systemAccounts:
        run(args.worbli + 'create account eosio ' + a + ' ' + args.public_key)
...
def createStakedAccounts(b, e):
    ...
    for i in range(b, e):
        ...
        retry(args.worbli + 'system newaccount --transfer eosio %s %s --stake-net "%s" --stake-cpu "%s" --buy-ram "%s"   ' % 
            (a['name'], a['pub'], intToCurrency(stakeNet), intToCurrency(stakeCpu), intToCurrency(ramFunds)))
        if unstaked:
            retry(args.worbli + 'transfer eosio %s "%s"' % (a['name'], intToCurrency(unstaked)))

def regProducers(b, e):
    for i in range(b, e):
        a = accounts[i]
        retry(args.worbli + 'system regproducer ' + a['name'] + ' ' + a['pub'] + ' https://' + a['name'] + '.com' + '/' + a['pub'])

def listProducers():
    run(args.worbli + 'system listproducers')
...
def vote(b, e):
    for i in range(b, e):
        ...
        retry(args.worbli + 'system voteproducer prods ' + voter + ' ' + prods)

def claimRewards():
    table = getJsonOutput(args.worbli + 'get table eosio eosio producers -l 100')
    times = []
    for row in table['rows']:
        if row['unpaid_blocks'] and not row['last_claim_time']:
            times.append(getJsonOutput(args.worbli + 'system claimrewards -j ' + row['owner'])['processed']['elapsed'])
    print('Elapsed time for claimrewards:', times)

def proxyVotes(b, e):
    vote(firstProducer, firstProducer + 1)
    proxy = accounts[firstProducer]['name']
    retry(args.worbli + 'system regproxy ' + proxy)
    sleep(1.0)
    for i in range(b, e):
        voter = accounts[i]['name']
        retry(args.worbli + 'system voteproducer proxy ' + voter + ' ' + proxy)

def updateAuth(account, permission, parent, controller):
    run(args.worbli + 'push action eosio updateauth' + jsonArg({
        'account': account,
...
def resign(account, controller):
    ...
    sleep(1)
    run(args.worbli + 'get account ' + account)
...
def randomTransfer(b, e):
    for j in range(20):
        ...
        run(args.worbli + 'transfer -f ' + src + ' ' + dest + ' "0.0001 ' + args.symbol + '"' + ' || true')
...
def msigProposeReplaceSystem(proposer, proposalName):
    ...
    run(args.worbli + 'multisig propose ' + proposalName + jsonArg(requestedPermissions) + 
        jsonArg(trxPermissions) + 'eosio setcode' + jsonArg(setcode) + ' -p ' + proposer)

def msigApproveReplaceSystem(proposer, proposalName):
    for i in range(firstProducer, firstProducer + numProducers):
        run(args.worbli + 'multisig approve ' + proposer + ' ' + proposalName +
            jsonArg({'actor': accounts[i]['name'], 'permission': 'active'}) +
            '-p ' + accounts[i]['name'])

def msigExecReplaceSystem(proposer, proposalName):
    retry(args.worbli + 'multisig exec ' + proposer + ' ' + proposalName + ' -p ' + proposer)

def msigReplaceSystem():
    run(args.worbli + 'push action eosio buyrambytes' + jsonArg(['eosio', accounts[0]['name'], 200000]) + '-p eosio')
    sleep(1)
...
def produceNewAccounts():
    with open('newusers', 'w') as f:
        for i in range(120_000, 200_000):
            x = getOutput(args.worbli + 'create key --to-console')
...
def stepInstallSystemContracts():
    run(args.worbli + 'set contract eosio.token ' + args.contracts_dir + 'eosio.token/')
    run(args.worbli + 'set contract eosio.msig ' + args.contracts_dir + 'eosio.msig/')
def stepCreateTokens():
    run(args.worbli + 'push action eosio.token create \'["eosio", "10000000000.0000 %s"]\' -p eosio.token' % (args.symbol))
    totalAllocation = allocateFunds(0, len(accounts))
    run(args.worbli + 'push action eosio.token issue \'["eosio", "%s", "memo"]\' -p eosio' % intToCurrency(totalAllocation))
    sleep(1)
def stepSetSystemContract():
    retry(args.worbli + 'set contract eosio ' + args.contracts_dir + 'eosio.system/')
    sleep(1)
    run(args.worbli + 'push action eosio setpriv' + jsonArg(['eosio.msig', 1]) + '-p eosio@active')
...
parser.add_argument('--worbli', metavar='', help="worbli command", default='../../build/programs/worbli/worbli --wallet-url http://127.0.0.1:6666 ')
...
parser.add_argument('-H', '--http-port', type=int, default=8000, metavar='', help='HTTP port for worbli')
...
args.worbli += '--url http://127.0.0.1:%d ' % args.http_port
